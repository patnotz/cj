...
int drive_simulation(Json::Value & config, int argc, char * argv[])
{
  ...
  Teuchos::RCP<Analysis_Model> analysis_model = analysis_model_factory.create(mesh_input_file_name, mesh_output_file_name);

//  Mesh_Manager mesh_manager = Mesh_Manager(mesh_input_file_name,
//    mesh_output_file_name);
  analysis_model->read_mesh();
  ...
  static stk::ParallelMachine parallel_machine = stk::parallel_machine_init(
    &argc, &argv);
  stk::mesh::STK_Mesh stk_mesh(parallel_machine, analysis_model->dimension());
  ...
  analysis_model->initialize_mesh_parts_and_commit(&stk_mesh);
  ...
  analysis_model->print_field_info(&stk_mesh);
  // put the elements and connectivity in the stk_mesh
  analysis_model->populate_mesh_elements(&stk_mesh);
  // put the coordinates as a field on the stk_mesh
  analysis_model->populate_mesh_coordinates(&stk_mesh);
  bool local_status = true;
  local_status = analysis_model->verify_coordinates_field(stk_mesh);
  stringstream oss;
  ...
  // temperature
  analysis_model->populate_bogus_scalar_field(&stk_mesh, temperature_field,
    stk_mesh.my_node_rank);
  // volume
  analysis_model->populate_bogus_scalar_field(&stk_mesh, volume_field,
    stk_mesh.my_elem_rank);
  // velocity
  analysis_model->populate_bogus_vector_field(&stk_mesh, velocity_field,
    stk_mesh.my_node_rank);
  // acceleration
  analysis_model->populate_bogus_vector_field(&stk_mesh, acceleration_field,
    stk_mesh.my_elem_rank);

  const char * title = resultsFileName.c_str();
  analysis_model->initialize_output(title, stk_mesh);
  ...
  for (int i = 0; i < num_time_steps; i++)
  {
    ...
    analysis_model->write_nodal_scalar(time_step, time_value, stk_mesh,
      temperature_field);
    analysis_model->write_nodal_vector(time_step, time_value, stk_mesh,
      stk_mesh.my_coordinates_field);
    analysis_model->write_nodal_vector(time_step, time_value, stk_mesh,
      velocity_field);

    analysis_model->write_element_scalar(time_step, time_value, stk_mesh,
      volume_field);
    analysis_model->write_element_vector(time_step, time_value, stk_mesh,
      acceleration_field);
  ...
  analysis_model->close_output_file();
