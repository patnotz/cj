...
static std::string codePointToUTF8(unsigned int cp)
{
  ...
  {
    result.resize(2);
    result[1] = static_cast<char>(0x80 | (0x3f & cp));result
    [0] = static_cast<char>(0xC0 | (0x1f & (cp >> 6)));}
    else if (cp <= 0xFFFF)
    {
      result.resize(3);
  ...
  bool collectComments)
{
  ...
  if (collectComments_ && !commentsBefore_.empty()) root.setComment(
    commentsBefore_, commentAfter);
...
bool Reader::readComment()
{
  ...
  if (collectComments_)
  {
    ...
    {
      if (c != '*' || !containsNewLine(commentBegin, current_)) placement =
...
bool Reader::decodeNumber(Token &token)
{
  ...
  {
    Char c = *current++;
    if (c < '0' || c > '9') return addError(
      "'" + std::string(token.start_, token.end_) + "' is not a number.", token);
    if (value >= threshold) return decodeDouble(token);
    value = value * 10 + Value::UInt(c - '0');
  }
  if (isNegative) currentValue() = -Value::Int(value);
  else if (value <= Value::UInt(Value::maxInt)) currentValue() = Value::Int(
    value);
  else
    currentValue() = value;
  return true;
...
bool Reader::decodeString(Token &token, std::string &decoded)
{
  ...
  while (current != end)
  {
    ...
    {
      if (current == end) return addError("Empty escape sequence in string",
        token, current);
      Char escape = *current++;
      switch (escape)
      {
  ...
  Location end, unsigned int &unicode)
{
  ...
  {
    // surrogate pairs
    if (end - current < 6) return addError(
...
bool Reader::decodeUnicodeEscapeSequence(Token &token, Location &current,
  Location end, unsigned int &unicode)
{
  if (end - current < 4) return addError(
...
std::string Reader::getLocationLineAndColumn(Location location) const
{
  ...
  char buffer[18 + 16 + 16 + 1];sprintf
  (buffer, "Line %d, Column %d", line, column);
  return buffer;
}
...
std::string Reader::getFormatedErrorMessages() const
{
  ...
  for (Errors::const_iterator itError = errors_.begin();
    ...
    if (error.extra_) formattedMessage += "See "
