...
public:
      ...
      unknown)
  {
    ...
    char *newString = static_cast<char *>(malloc(length + 1) );memcpy
...
Value::CZString::CZString(const char *cstr, DuplicationPolicy allocate) :
    cstr_(
      allocate == duplicate ? valueAllocator()->makeMemberName(cstr) : cstr), index_(
      allocate)
{
}
...
Value::CZString::CZString(const CZString &other) :
    cstr_(
      other.index_ != noDuplication && other.cstr_ != 0 ?
        valueAllocator()->makeMemberName(other.cstr_) : other.cstr_), index_(
      other.cstr_ ? (other.index_ == noDuplication ? noDuplication : duplicate)
      : other.index_ ){
    }

Value::CZString::~CZString()
{
  if (cstr_ && index_ == duplicate) valueAllocator()->releaseMemberName(
...
#endif
{
  ...
  if (other.comments_)
  {
    ...
    for (int comment = 0; comment < numberOfCommentPlacement; ++comment)
    {
      ...
      if (otherComment.comment_) comments_[comment].setComment(
...
Value::~Value()
{
  switch (type_)
  {
    ...
    case booleanValue:
      break;
    case stringValue:
      if (allocated_) valueAllocator()->releaseStringValue(value_.string_);
      break;
#ifndef JSON_VALUE_USE_INTERNAL_MAP
    case arrayValue:
    case objectValue:
...
void Path::makePath(const std::string &path, const InArgs &in)
{
  ...
  while (current != end)
  {
    ...
    {
      ++current;
      if (*current == '%') addPathInArg(path, in, itInArg,
      ...
      if (current == end || *current++ != ']') invalidPath(path,
